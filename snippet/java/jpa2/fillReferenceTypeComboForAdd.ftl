...
<#assign projectSpecificSearchProperty = generator.getElementProperty(currentModelReference, "projectspecific.fillComboForAdd", "false")>
<#assign projectSpecificSearch = generator.elementContainsProperty(currentModelReference, "projectspecific.fillComboForAdd", "true")>
<#if projectSpecificSearch >
  <#assign body>
    <@createBodyUsingJPA2 />
  </#assign>
  ${generator.evaluateFreeMarkerSnippet("projectspecific.java.cdi.beans.fillReferenceTypeComboForAdd", body)}
<#else>
  <@createBodyUsingJPA2 />
</#if>
...
-->
  ...
  <#local contextForAdd = "contextForAdd">
  ...
  <#list references1 as objectReference>
    ...
    <#-- Als $referenceObjectElement nu ook een reference heeft van dit type, dan moet het worden toegevoegd aan criteria -->
      <#if (generator.findChildByAttribute(referenceObjectElement, "reference", "type", objectReferenceType))?? >
        ...
        if (${contextForAdd}.get${objectReferenceNameFU}() != null)
        {
          ${generator.addLibraryToClass("${packageName}.${referenceClassName}_")}
          <#if isEnum=="true">
            <#local equalName = "equal${sameReferenceNameFU}">
            Expression<Boolean> ${equalName} = cb.equal(${fromReference}.get(${referenceClassName}_.${objectReferenceName}), ${contextForAdd}.get${objectReferenceNameFU}());
          ...
          <#else>
            ...
            Expression<Boolean> ${equalName} = cb.equal(${joinName}, ${contextForAdd}.get${objectReferenceNameFU}());
            allCriteria = cb.and(allCriteria, ${equalName});
          </#if>

          //criteria.add(Restrictions.eq("${sameReferenceName}", ${contextForAdd}.get${objectReferenceNameFU}()));
        }
